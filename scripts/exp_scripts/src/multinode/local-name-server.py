#!/usr/bin/env python
#!/home/abhigyan/Python-2.7.3/bin/python
...
local_name_server_jar = exp_config.jar_file_remote              #Local name server jar
...
regular_workload = 11000                              #Size of regular workload, seems not used for local name server if update is not sent
...
numberOfTransmissions = exp_config.numberOfTransmissions     # maximum number of times a query is transmitted
...
def run_local_name_server():
    ...
    if not lookup_trace_file  == '': # and os.path.exists(lookup_trace_file):
        command += ' ' + LOOKUP_TRACE_FILE + ' ' + lookup_trace_file
        
    if not update_trace_file  == '': # and os.path.exists(update_trace_file):
...
def print_options():
    if is_debug_mode:
        ...
        print "Update Rate: " + str(update_rate_regular) + "ms"
    ...
    try:                                
        opts, args = getopt.getopt(argv, 'ab:c:d:e:fg:h:i:jk:l:m:n:o:p:qr:tuv:w:x:y:z:s:', 
                                   ['local', 'id=', 'nsFile=', 'lnsFile=',
                                    ...
                                    'expRunTime=' , 'rInterval=', 'optimalTrace=', 'ip='])
