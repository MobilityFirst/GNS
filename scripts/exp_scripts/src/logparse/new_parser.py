...
# GLOBAL VARIABLES
p = 0
initial_fraction = 0.5             # exclude initial fraction of requests
final_fraction = 1.0                # parse queries up to the final fraction of responses.

success = 0                         # total number of successful requests
...
def update_tuples(input_dict, tokens):
    ...
    try:
        if tokens[1].endswith('Query'):
            ...
            input_dict[int(tokens[2])] = [int(tokens[2]),tokens[3],tokens[4],tokens[5], float(tokens[6]), int(tokens[7]), -1, failed_latency]
        else :
            ...
            if query_id in input_dict:
                input_dict[query_id][6] = int(tokens[7])
