...
public class SelectTest extends DefaultGNSTest {
  ...
  public void test_040_GeoSpatialSelect() {
    try {
      for (int cnt = 0; cnt < 5; cnt++) {
    	String name = null;
  		client.execute(GNSCommand.createGUID(masterGuid, name="geoTest-" + RandomString.randomString(12)));
        GuidEntry testEntry = GuidUtils.getGUIDKeys(name);
        CREATED_GUIDS.add(testEntry); // save them so we can delete them later
        client.execute(GNSCommand.setLocation(testEntry, 0.0, 0.0));
        waitSettle(WAIT_SETTLE);
      }
    } catch (ClientException | IOException e) {
      Utils.failWithStackTrace("Exception when we were not expecting it: " + e);
    ...
    try {
      ...
      loc.put(1.0);
      loc.put(1.0);
			JSONArray result = client.execute(
					GNSCommand.selectNear(masterGuid,
							GNSProtocol.LOCATION_FIELD_NAME.toString(), loc,
							2000000.0)).getResultJSONArray();
    ...
    try {
      ...
      rect.put(lowerRight);
			JSONArray result = client.execute(
					GNSCommand.selectWithin(masterGuid,
							GNSProtocol.LOCATION_FIELD_NAME.toString(), rect))
					.getResultJSONArray();
      // best we can do should be at least 5, but possibly more objects in results
      Assert.assertThat(result.length(), Matchers.greaterThanOrEqualTo(5));
    } catch (JSONException | ClientException | IOException e) {
  ...
  public void test_080_SelectPass() {
    try {
      waitSettle(WAIT_SETTLE);
      JSONArray result = client.execute(GNSCommand.selectQuery(masterGuid, buildLocationQuery(createIndexTestField, AREA_EXTENT))).getResultJSONArray();
      for (int i = 0; i < result.length(); i++) {
        System.out.println(result.get(i).toString());
      }
      // best we can do should be at least 5, but possibly more objects in results
