...
public class ServerIntegrationTest {
		...
		do {
			try {
				System.out.println("Connectivity check: " + (tries - 1)
						+ " attempt remaining.");
				client.checkConnectivity(); // this will throw the IOException
	...
	// arun: Added TestWatcher and TestName and other pretty printing
	@Rule
	...
	public void test_190_Substitute() {
		...
		try {
			...
			testEntry = GuidUtils.registerGuidWithTestTag(client, masterGuid,
					testSubstituteGuid);
	...
	public void test_270_RemoveField() {
		...
		try {
			...
			fail("Exception while reading the field " + fieldToDelete + ": "
					+ e);
		}
		try {
	...
	public static void main(String[] args) {
		...
		System.out.println("\n\n-----------Completed all " + result.getRunCount() + " tests"
				+ (result.getFailureCount() > 0 ? "; the following "
						+ result.getFailureCount() + " tests failed:" : " successfully")
				+ "--------------");
