...
public class Format {
  ...
  /**
   * TIMESTAMP in UTC with milleseconds
   */
  public final static ThreadLocal<SimpleDateFormat> formatDateMilleUTC =
          new ThreadLocal<SimpleDateFormat>() {
            @Override
            protected SimpleDateFormat initialValue() {
              SimpleDateFormat result = new SimpleDateFormat("yyyyMMddHHmmssSSS");
              result.setTimeZone(TimeZone.getTimeZone("UTC"));
              return result;
            }
          };
   ...
   * @return
   */
  public static String formatDateMilleUTC(Date date) {
    return formatDateMilleUTC.get().format(date);
  }

  public static Date parseDateMilleUTC(String text) throws ParseException {
    return formatDateMilleUTC.get().parse(text);
  }
  /**
   * SHORT DATE
   */
