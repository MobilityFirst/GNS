...
public class Geometry {
    ...
    public static Point2D.Double chop(Shape shape, Point2D.Double p) {
        ...
        for (; !i.isDone(); i.next()) {
            ...
            Point2D.Double chop = Geometry.intersect(
                    prevX, prevY,
                    coords[0], coords[1],
                    p.x, p.y,
                    ctr.x, ctr.y);
            ...
            if (chop != null) {
                double cl = Geometry.length2(chop.x, chop.y, p.x, p.y);
                if (cl < len) {
                    len = cl;
                    cx = chop.x;
                    cy = chop.y;
        ...
        if (len == Double.MAX_VALUE) {
            ...
            for (; !i.isDone(); i.next()) {
                ...
                double l = Geometry.length2(ctr.x, ctr.y, coords[0], coords[1]);
                if (l < len) {
                    len = l;
                    cx = coords[0];
                    cy = coords[1];
    ...
    public static Point ovalAngleToPoint(Rectangle r, double angle) {
        Point center = Geometry.center(r);
        Point p = Geometry.polarToPoint(angle, r.width / 2, r.height / 2);
    ...
    public static Point2D.Double ovalAngleToPoint(Rectangle2D.Double r, double angle) {
        Point2D.Double center = Geometry.center(r);
        Point2D.Double p = Geometry.polarToPoint2D(angle, r.width / 2, r.height / 2);
    ...
    public static double distanceFromLine(int xa, int ya,
        ...
        if (l2 == 0) {
            return Geometry.length(xa, ya, xc, yc);
        }
   ...
   * @param L
   */
  public static double parameterOfNearestPointOnLine(Point2D j, Point2D k, Point2D l) {
    return parameterOfNearestPointOnLine(j.getX(), j.getY(), k.getX(), k.getY(), l.getX(), l.getY());
  }
  
  public static double parameterOfNearestPointOnLine(double xj, double yj, double xk, double yk, double xl, double yl) {
    Point2D FandG = segmentToParametricLine(xk, yk, xl, yl);
    double f = FandG.getX();
    double g = FandG.getY();
    double xkj = xk - xj;
    double ykj = yk - yj;

    double denom = f * f + g * g;
    if (denom == 0) {
      // denom=0 means K=L, so we can really give either 0.0 or 1.0
      return 0.0;
    } else {
      return (-((xkj * f) + (ykj * g))) / denom;
    }
  }

  /**
   * The distance squared from point J to the line segment K-L.
   *
   */
  public static Point2D nearestPointOnSegment(Point2D j, Point2D k, Point2D l) {
   return nearestPointOnSegment(j.getX(), j.getY(), k.getX(), k.getY(), l.getX(), l.getY());
  }
   ...
   *
   */
  public static Point2D nearestPointOnSegment(double xj, double yj, double xk, double yk, double xl, double yl) {
    double tt = parameterOfNearestPointOnLine(xj, yj, xk, yk, xl, yl);
    if (tt > 1.0) {
      tt = 1.0;
    }
    if (tt < 0.0) {
    ...
    return segmentAndParameterToPoint(xk, yk, xl, yl, tt);
  }
   ...
   * @return
   */
  public static double pointDistanceFromSegment(Point2D j, Point2D k, Point2D l) {
    Point2D tmp = nearestPointOnSegment(j, k, l);
    return tmp.distance(j);
  }
   ...
   *
   */
  public static Point2D segmentToParametricLine(Point2D p1, Point2D p2) {
    return segmentToParametricLine(p1.getX(), p1.getY(), p2.getX(), p2.getY());
  }

  public static Point2D segmentToParametricLine(double x1, double y1, double x2, double y2) {
   ...
   * @return
   */
  public static Point2D segmentAndParameterToPoint(Point2D p1, Point2D p2, double parameter) {
    return segmentAndParameterToPoint(p1.getX(), p1.getY(), p2.getX(), p2.getY(), parameter);
  }
   ...
   * @return
   */
  public static Point2D segmentAndParameterToPoint(double x1, double y1, double x2, double y2, double parameter) {
    Point2D FandG = segmentToParametricLine(x1, y1, x2, y2);
    return new Point2D.Double(x1 + parameter * FandG.getX(), y1 + parameter * FandG.getY());
  }

  public static double angleBetweenPoints(Point2D p1, Point2D p2) {
    return angleBetweenPoints(p1.getX(), p1.getY(), p2.getX(), p2.getY());
  }
