...
public class MultiArrayMap<KeyType,ValueType extends Keyable<KeyType>> {
	...
	private static Logger log = Logger.getLogger(MultiArrayMap.class.getName()); // GNS.getLogger();
	...
	public synchronized void put(KeyType key, ValueType value) {
		...
		for(Object[] array : this.aMap) {
			...
			if(array[index]==null || 
					(array[index] instanceof Keyable<?> && key.equals(((Keyable<?>)array[index]).getKey())))
			{
				if(array[index]==null) count++; 
				else log.info("Overwrote " + value + " in level " + level + " index " + index);
				array[index] = value;
				inserted=true;
				break;
			}
	...
	public static void main(String[] args) {
		...
		if(simpleArray) {
			svarray = new StringValue[size];
			for(int i=0; i<size; i++) {
				svarray[i] = new StringValue<Integer>("paxos"+i, i+23);
			}
		}
		else {
			map = new MultiArrayMap<String,StringValue<Integer>>((int)(size), 6);
