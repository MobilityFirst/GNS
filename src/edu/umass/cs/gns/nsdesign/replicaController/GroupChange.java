    ...
    try {
      ...
      if (!rcRecord.isActiveRunning()) {
        ...
        if (!recovery && trackGroupChange.size() > 0) {
          InetSocketAddress lnsAddress = activeProposalPacket.getCCPAddress();
      ...
      if (!recovery) {
        if (activeProposalPacket.getProposingNode().equals(replicaController.getNodeID()) && activeProposalPacket.getCCPAddress() != null) {
          GNS.getLogger().info("Putting packet in hash map: " + activeProposalPacket);
          trackGroupChange.put(new GroupChangeIdentifier(activeProposalPacket.getName(), activeProposalPacket.getVersion()), activeProposalPacket.getCCPAddress());
        }
  ...
  public static void handleNewActiveStartConfirmMessage(NewActiveSetStartupPacket packet, ReplicaController replicaController)
    ...
    if (o != null) {
      ...
      OldActiveSetStopPacket oldActiveSetStopPacket = new OldActiveSetStopPacket(packet.getName(), 0,
              replicaController.getNodeID(), null, packet.getOldActiveVersion(), Packet.PacketType.DELETE_OLD_ACTIVE_STATE);
