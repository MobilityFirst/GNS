...
public class TESTPaxosConfig {
  ...
  static {
    setDefaultGroups(MAX_CONFIG_GROUPS);
  }
  ...
  static {
    assert (NUM_CLIENTS <= MAX_CONFIG_GROUPS);
  ...
  public static void createGroup(String groupID, int[] members) {
    if (groups.size() <= MAX_CONFIG_GROUPS) {
      groups.put(groupID, members);
    }
  }
  ...
  public static void testAssert(boolean b) {
    assert (!TEST_ASSERT_ENABLED || b);
  }
