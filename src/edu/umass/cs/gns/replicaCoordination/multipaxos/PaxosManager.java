...
public class PaxosManager<NodeIDType> extends AbstractPaxosManager<NodeIDType> {
  ...
  public PaxosManager(NodeIDType id, InterfaceNodeConfig<NodeIDType> nc, InterfaceJSONNIOTransport<NodeIDType> niot, Replicable pi, PaxosConfig pc) {
    ...
    if (niot.getClass().equals(JSONNIOTransport.class)) {
      ...
      if (!((JSONNIOTransport) niot).isStarted()) {
        (new Thread(((JSONNIOTransport) niot))).start();
      }
    }
    initiateRecovery();
  }
    ...
    try {
      ...
      switch (paxosPacketType) {
        ...
        default:
          ...
          if (pism == null) {
            log.severe("Node " + myID + " unable to find paxos instance for " + paxosID + ":" + jsonMsg);
          }
          break;
      }
    } catch (JSONException je) {
      log.severe("Node " + this.myID + " received bad JSON message: " + jsonMsg);
      je.printStackTrace();
    }
    setProcessing(false);
  }
  ...
  private synchronized boolean restore(String paxosID) {
    ...
    if (HIBERNATE_OPTION) {
      ...
      if (pri != null) {
        pism = this.recover(paxosID, pri.getVersion(), this.myID, pri.getMembers(), this.myApp, true);
      }
      if (pism != null) {
        restored = true;
        log.info("Node" + myID + " successfully restored hibernated instance " + paxosID);
      } else {
        log.warning("Node" + myID + " unable to restore instance " + paxosID);
      }
    }
    return restored;
  }
  ...
  private synchronized boolean unpause(String paxosID) {
    ...
    if (PAUSE_OPTION) {
      ...
      if (hri != null) {
        ...
        log.fine("Node" + myID + " unable to hot restore instance " + paxosID);
      }
    }
    return restored;
  }
  ...
  private PaxosInstanceStateMachine recover(String paxosID, short version, int id, int[] members, Replicable app, boolean restoration) {
    if (DEBUG) {
      log.info("Node " + this.myID + " " + paxosID + ":" + version + " recovering");
    }
  ...
  private void rollForward(String paxosID) {
    if (!ONE_PASS_RECOVERY || this.hasRecovered()) {
      if (DEBUG) {
        log.info("Node " + this.myID + " " + paxosID + " about to roll forward: ");
      }
      AbstractPaxosLogger.rollForward(paxosLogger, paxosID, messenger);
      ActivePaxosState activeState = this.getActiveState(false, paxosID);
      if (activeState != null) {
        activeState.setRecovered();
      }
      PaxosInstanceStateMachine pism = (this.getInstance(paxosID, true, false));
      assert (this.getInstance(paxosID, false, false) != null);
      if (pism != null) {
   ...
   */
  private void monitor(Set<Integer> nodes) {
    FD.monitor(nodes);
  }
  /* Stop sending pings to the node */

  private void unMonitor(int id) {
    FD.unMonitor(id);
  }
  ...
  private void findReplicaGroup(JSONObject msg, String paxosID, short version) throws JSONException {
    ...
    if (nodeID >= 0) {
      ...
      log.severe("Can't find group member in paxosID:version " + paxosID + ":" + version);
    }
  }
  ...
  protected ActivePaxosState getActiveState(boolean active, String paxosID) {
    synchronized (this.activePaxii) {
      ...
      if (activeState == null) {
        this.activePaxii.put(paxosID, (activeState = new ActivePaxosState(paxosID)));
      }
      if (active) {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      while (true) {
        boolean done = false;
        for (int i = 0; i < nNodes; i++) {
          if (apps[i].getNumCommitted(GUIDs[0]) >= maxRecoverySlot + numReqs) {
            done = true;
          }
        }
        if (done) {
