...
//    }).start();
  ...
  static {
    try {
      cppAddress = new InetSocketAddress(NetworkUtils.getLocalHostLANAddress().getHostAddress(), GNS.DEFAULT_LNS_TCP_PORT);
      GNS.getLogger().info("CPP Address is " + cppAddress);
    } catch (UnknownHostException e) {
      GNS.getLogger().info("Unabled to determine CPP address: " + e);
      cppAddress = new InetSocketAddress(InetAddress.getLoopbackAddress(), GNS.DEFAULT_LNS_TCP_PORT);
    }
  }
