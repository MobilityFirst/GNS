...
public class Remove {
   ...
   */
  protected static MessagingTask[] executeMarkRecordForRemoval(RemoveRecordPacket removeRecord, BasicRecordMap DB, NodeId<String> rcID,
   ...
   */
  protected static MessagingTask handleActiveRemoveRecord(OldActiveSetStopPacket activeStop, NodeId<String> rcID, UniqueIDHashMap map,
    ...
    if (Config.debuggingEnabled) {
      log.fine("RC handling active remove record ... " + activeStop);
    }
    ...
    if (Config.debuggingEnabled) {
      log.fine("RC remove packet fetched ... " + removePacket);
    }
   ...
   *
   */
  protected static MessagingTask[] executeRemoveRecord(RemoveRecordPacket removeRecordPacket, BasicRecordMap DB, NodeId<String> rcID,
          boolean recovery) throws JSONException, FailedDBOperationException, IOException {
    if (Config.debuggingEnabled) {
      log.fine("DECISION executing remove record at RC: " + removeRecordPacket);
    }
    ...
    MessagingTask replyToLNS = null;
    if (removeRecordPacket.getNameServerID().equals(rcID)) { // this will be true at the replica controller who
      ...
      if (Config.debuggingEnabled) {
        log.fine("Remove record response sent to LNS: " + removeRecordPacket.getName() + " lns "
