...
public class ExampleNode {
  ...
  private final Set<NodeId<String>> nodeIDs;
  private final JSONNIOTransport<NodeId<String>> niot;
  private final ProtocolExecutor<NodeId<String>, Packet.PacketType, Long> protocolExecutor;

  private Logger log
          = NIOTransport.LOCAL_LOGGER ? Logger.getLogger(getClass().getName())
          : GNS.getLogger();

  ExampleNode(NodeId<String> id, InterfaceNodeConfig<NodeId<String>> nc) throws IOException {
    // Setting my ID and NIO with packet demultiplexer
    this.myID = id;
    this.nodeIDs = (nc.getNodeIDs());
    this.niot
            = new JSONNIOTransport<NodeId<String>>(id, nc, new ExamplePacketDemultiplexer(
                            this), true);

    // protocol executor
    this.protocolExecutor
            = new ProtocolExecutor<NodeId<String>, Packet.PacketType, Long>(
                    new JSONMessenger<NodeId<String>>(niot));
  ...
  protected NodeId<String> getMyID() {
    return this.myID;
  }
   ...
   * above.
   */
  protected PingPongProtocolTask reliableSend( NodeId<String> id) {
    TreeSet< NodeId<String>> nodes = new TreeSet< NodeId<String>>();
    nodes.add(id);
    PingPongProtocolTask task
   ...
   * it.
   */
  protected ThresholdProtocolTask< NodeId<String>, Packet.PacketType, Long> thresholdFetch() {
  ...
  public static void main(String[] args) {
    int startID = 100;
    int numNodes = 2;
    SampleNodeConfig<NodeId<String>> snc = new SampleNodeConfig<NodeId<String>>();
    for (int i = startID; i < startID + numNodes; i++) {
      snc.addLocal(new NodeId<String>(i));
    }
    ExampleNode[] nodes = new ExampleNode[numNodes];
    try {
      for (int i = 0; i < numNodes; i++) {
        nodes[i] = new ExampleNode(new NodeId<String>(startID + i), snc);
      }
