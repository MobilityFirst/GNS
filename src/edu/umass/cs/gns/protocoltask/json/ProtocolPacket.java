 ...
 * messages, but the PacketType is left generic. The reason is so that
 * implementors can define and use their own packet types. Note that it
 * is not possible to extend the enum Packet.PacketType and it is not 
 * modular to have to put *all* packet types in the Packet class.
 */
public abstract class ProtocolPacket<NodeIDType,EventType> implements ThresholdProtocolEvent<NodeIDType,EventType,String> {
	
	public static enum Keys {SENDER, INITIATOR, KEY}; // all protocol packets carry these fields
	...
	public ProtocolPacket(JSONObject json) throws JSONException {
		...
		this.sender = (json.has(Keys.SENDER.toString()) ? (NodeIDType)(json.get(Keys.SENDER.toString())) : null);
		this.initiator = (json.has(Keys.INITIATOR.toString()) ? (NodeIDType)(json.get(Keys.INITIATOR.toString())) : null);
