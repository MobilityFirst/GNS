...
public class JSONMessageExtractor implements InterfaceDataProcessingWorker {
  ...
  private synchronized ArrayList<JSONObject> parseAndSetSockStreams(
    ...
    try {
      ...
      int port = ((InetSocketAddress) sock.getRemoteAddress()).getPort();
      stampAddressIntoJSONObjects(address, port, jsonArray);
    } catch (IOException e) {
  ...
  private void stampAddressIntoJSONObjects(String address, int port, ArrayList<JSONObject> jsonArray) {
    try {
