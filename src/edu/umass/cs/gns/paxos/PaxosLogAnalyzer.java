...
public class PaxosLogAnalyzer {
   ...
   */
  HashMap<Integer, ConcurrentHashMap<String, PaxosReplica>> paxosAllNS;
  ...
  public PaxosLogAnalyzer(String paxosLogFolder, int numNS) {
    ...
    this.numNS = numNS;
    paxosAllNS = new HashMap<Integer, ConcurrentHashMap<String, PaxosReplica>>();
  }
  ...
  public void readLogData() {
    ...
    for (int nodeID = 0; nodeID < numNS; nodeID++) {
      ...
      ConcurrentHashMap<String, PaxosReplica> paxosInstances = PaxosLogger.readAllPaxosLogs(nodeID);
  ...
  public void outputPaxosStats(String outputFile, String failedNodesFile) {
    ...
    for (String paxosID: paxosNodeIDs.keySet()) {
      ...
      for (Integer nodeID: paxosNodeIDs.get(paxosID)) {
        PaxosReplica replica = paxosAllNS.get(nodeID).get(paxosID);
...
//            System.err.println("PaxosID\t" + paxosID + "\tnot created at node\t" + nodeID);
        ...
        if (acceptCount < paxosGroupSize || commitCount < paxosGroupSize) {
          ...
          if (slot == slotCount - 1 && lastRequestStop) isStop = true;
...
//                  (paxosGroupSize - acceptCount) + "\t"  + (paxosGroupSize - commitCount));
    ...
    System.out.println("paxosIDSlots count\t" + paxosIDSlots.size());

    File f = new File(outputFile);
    String parentFolder = f.getParent();
    if (parentFolder != null) { // create parent folder if necessary
  ...
  public HashSet<Integer> readFailedNodes(String failedNodesFile) {
    try {
      ...
      if (f.isFile()) {
        ...
        if (line == null) return failedNodes;
