...
/*
 * This class is a wrapper around NodeConfig to ensure that it is consistent, i.e., it returns
 * consistent results even if it changes midway. In particular, it does not allow the use of a
 * method like getNodeIDs().
 * 
 * It also has consistent hashing utility methods.
 */
public class ConsistentReconfigurableNodeConfig<NodeIDType> extends ConsistentNodeConfig<NodeIDType> implements
	...
	private final ConsistentHashing<NodeIDType> CH_RC; // need to refresh when nodeConfig changes
	private final ConsistentHashing<NodeIDType> CH_AR; // need to refresh when nodeConfig changes

	public ConsistentReconfigurableNodeConfig(InterfaceReconfigurableNodeConfig<NodeIDType> nc) {
		super(nc);
		this.nodeConfig = nc;
	...
	public NodeIDType getFirstReconfigurator(String name) {
		this.refreshReconfigurators();
		Set<NodeIDType> reconfigurators = this.CH_RC.getReplicatedServers(name);
	 ...
	 * is maximal overlap with the specified set of nodes, oldNodes. It is somewhat
	 * nontrivial only because there is a many-to-one mapping from nodes to addresses, so
	 * a simple reverse lookup is not meaningful.
	 * 
	 */
	public Set<NodeIDType> getIPToNodeIDs(ArrayList<InetAddress> newAddresses, Set<NodeIDType> oldNodes) {
