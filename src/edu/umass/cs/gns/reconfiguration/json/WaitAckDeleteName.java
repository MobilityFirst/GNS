 ...
 * StopEpoch messages.
 */
public class WaitAckDeleteName<NodeIDType> extends
	...
	public WaitAckDeleteName(NodeIDType sender, String name, Set<NodeIDType> actives) {
		super(actives); // default is all?
		this.deleteName = new DropEpochFinalState<NodeIDType>(sender, 
				name, 0, true);
		this.group = actives;
	}
	...
	public GenericMessagingTask<NodeIDType, ?>[] start() {
		...
		GenericMessagingTask<NodeIDType, ?> mtask = new GenericMessagingTask<NodeIDType, 
				DropEpochFinalState<NodeIDType>>(this.group.toArray(), this.deleteName);
		return mtask.toArray();
	}

	@Override
	public String refreshKey() {
		return (this.key = Util.refreshKey(this.deleteName.getSender().toString()));
	}
