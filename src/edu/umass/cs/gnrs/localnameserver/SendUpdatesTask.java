...
//	int MAX_TIMEOUTS = 3;
	...
	public void run()
	{
        ...
        int nameServerID = LocalNameServer.getClosestActiveNameServerFromCache(name, //nameRecordKey, 
                activesQueried);
		if (LocalNameServer.isValidNameserverInCache(name//, nameRecordKey
                        )) {
			if (nameServerID == -1) {
				if (StartLocalNameServer.debugMode) GNS.getLogger().fine("ERROR: No more actives left to query RETURN. Actives Queried " + activesQueried);
				return;
			}
			else {
