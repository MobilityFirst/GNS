 ...
 * @version 1.0
 */
/*public class BackgroundWritingThreadBackUp implements Runnable
{
  ...
  private static Logger      log                         = Logger.getLogger(BackgroundWritingThread.class.getName());

  public BackgroundWritingThreadBackUp(ConnectionInfo cinfo)
  {
    this.cinfo = cinfo;
    log.trace(cinfo.getServerOrClient() + "getDataBaseSeq num " + cinfo.getDataBaseSeq());
  ...
  {
    int iterationNum = 0;
    log.trace("BackgroundWritingThread started");
    while (runningStatus && (cinfo.getMSocketState() != MSocketConstants.CLOSED))
    {
      log.trace("BackgroundWritingThread iteration " + iterationNum);
      //while (!outstandingBytesOnAnyPath())
      //{
        log.trace("outstandingBytesOnAnyPath lastRetransmitSendSeqNum " + lastRetransmitSendSeqNum + "data send "
            + cinfo.getDataSendSeq());
        synchronized ( cinfo.getBackgroundThreadMonitor() )
        {
          try
          {
            cinfo.getBackgroundThreadMonitor().wait();
          }
        ...
        //if ( !runningStatus || (cinfo.getMSocketState() == MSocketConstants.CLOSED) )
        //  break;
        //}
    		  ...
    		  (cinfo.getMSocketState() != MSocketConstants.CLOSED))
      {
        ...
        if (byteObj != null)
        {
          log.trace("sending seq num " + byteObj.getStartSeqNum() + "getDataBaseSeq num " + cinfo.getDataBaseSeq()
          ...
          do {
              ...
              log.trace("READ_WRITE lock acquired in the Background thread");
      ...
      //System.out.println("One iteration finishes " + "finish path time " + (finishPath - runStart) + " retransmit time "
      //    + (retransmitEnd - finishPath) + " totalRetransmitBytes " + totalRetransmitBytes
      //    + " lastRetransmitSendSeqNum " + lastRetransmitSendSeqNum);
      iterationNum++;
    }
    
    log.debug("BackgroudMultiPathWritingThread stopped totalRetransmit " + totalRetransmitBytes);
  }
  ...
  private boolean retransmitChunk(byte[] retransmitData, ByteRangeInfo byteObj)
  {
        ...
        && (cinfo.getMSocketState() != MSocketConstants.CLOSED))
    {
      ...
      {
        log.debug( "length " + length + " tempDataSendSeqNum " + tempDataSendSeqNum
  ...
  private void getTheFasterPath(Vector<SocketInfo> socketList)
  {
    ...
    for (int i = 0; i < socketList.size(); i++)
    {
      ...
      if (Obj.getStatus())
      {
        ...
        {
          log.trace("socket ID " + Obj.getSocketIdentifer() + " is found to be faster path " + "Remote Address "
        ...
        else
        {
          ...
          // log.trace("unifinished paths size " + unfinishedPaths.size());
        }
      }
    }
  }
  ...
  private void waitForOnePathToFinish()
  {
    while (cinfo.getDataBaseSeq() < cinfo.getDataSendSeq())
    {
      log.trace("waitForOnePathToFinish");
      Vector<SocketInfo> socketList = new Vector<SocketInfo>();
      ...
      for (int i = 0; i < socketList.size(); i++)
      {
        ...
        if (Obj.getStatus())
        {
          ...
          {
            log.debug("socket ID " + Obj.getSocketIdentifer() + " is found to have zero outstanding bytes "
          ...
          else
          {
            unfinishedPaths.add(Obj);
            log.trace("unifinished paths size " + unfinishedPaths.size());
            log.debug("socket ID " + Obj.getSocketIdentifer() + " waitForOnePathToFinish " + "Remote Address "
                + Obj.getSocket().getInetAddress() + " sent bytes " + Obj.getSentBytes()
                + " Obj.getOutStandingBytes() " + Obj.getOutStandingBytes());
          }
        }
      ...
      if (finishedPaths.size() > 0)
      {
        ...
        if (finishedPaths.size() > 1)
        {
          ...
          log.debug("socket ID " + minOutBytesPath.getSocketIdentifer() + " added to finished path " + " sent bytes "
          ...
          for (int i = 0; i < socketList.size(); i++)
          {
            ...
            {
              log.debug("socket ID " + socketList.get(i).getSocketIdentifer() + " added to unfinished path "
