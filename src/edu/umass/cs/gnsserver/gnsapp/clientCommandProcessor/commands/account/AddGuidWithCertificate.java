 ...
 * @author westy
 */
public class AddGuidWithCertificate extends AbstractCommand {
	 ...
	 * @param module
	 */
	public AddGuidWithCertificate(CommandModule module) {
		super(module);
	}
	...
	public CommandType getCommandType() {
		return CommandType.AddGuidWithCertificate;
	}
	...
	public CommandResponse execute(InternalRequestHeader header, CommandPacket commandPacket, ClientRequestHandlerInterface handler) throws InvalidKeyException, InvalidKeySpecException,
		...
		String signature = json.optString(GNSProtocol.SIGNATURE.toString(),
			null);
		String message = json.optString(GNSProtocol.SIGNATUREFULLMESSAGE
			.toString(), null);
		String certificate = json.getString(GNSProtocol.CERTIFICATE.toString());
		...
		if ((signature!=null && NSAccessSupport.verifySignature(accountGuidInfo
			.getPublicKey(), signature, message))

			||

			// verify certificate
			verifyCertificate(name, publicKey, certificate, accountGuidInfo.getName())
	...
	private boolean verifyCertificate(String name, String publicKey, String
		certificate, String name1) {
