...
import edu.umass.cs.gnsserver.localnameserver.LocalNameServerOptions;
...
import edu.umass.cs.gnsserver.nodeconfig.NodeInfo;
...
public class LNSNodeConfig implements NodeConfig<InetSocketAddress>,
	...
	public LNSNodeConfig() {
		...
		hostInfoMapping = new ConcurrentHashMap<Object, LNSNodeInfo>();
		...
		GNSConfig.getLogger().info("LNS mapping = " + this.hostInfoMapping);
		this.hostsFile = null;
	}
	...
	public void updatePingLatency(InetSocketAddress address, long responseTime) {
		...
		if (nodeInfo != null) {
			...
			GNSConfig.getLogger().warning(
					"Can't update latency for " + address.toString() + ".");
		}
	}
	...
	public int getPingPort(InetSocketAddress id) {
		...
		if (nodeInfo != null) {
			return nodeInfo.getStartingPortNumber()
					+ GNSConfig.PortType.NS_PING_PORT.getOffset();
		} else {
			return INVALID_PORT;
		}
	}
	...
	public boolean nodeExists(InetSocketAddress address) {
		return address instanceof InetSocketAddress
				&& getNodeInfoForAnyNode((InetSocketAddress) address) != null;
	}
	...
	private void readHostsFile(String hostsFile) throws IOException {
		...
		// bogus
		ConcurrentMap<Object, LNSNodeInfo> newHostInfoMapping = new ConcurrentHashMap<Object, LNSNodeInfo>(
				16, 0.75f, 8);
	...
	private void startCheckingForUpdates() {
		...
		t.scheduleAtFixedRate(timerTask = new TimerTask() {
			...
			public void run() {
				...
				updateCheckPeriod);
		GNSConfig.getLogger().info(
				"Checking for hosts updates every " + updateCheckPeriod / 1000
						+ " seconds");
	}
