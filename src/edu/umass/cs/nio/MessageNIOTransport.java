...
public class MessageNIOTransport<NodeIDType, MessageType> extends
	...
	// didn't add this to "stamping" code below but it is declared for other
	// uses
	...
	public MessageNIOTransport(NodeIDType id,
		...
		super(id, nodeConfig, new JSONMessageExtractor(pd));
		if (start && !isStarted()) {
	...
	protected MessageNIOTransport(NodeIDType id,
			InterfaceNodeConfig<NodeIDType> nodeConfig,
	...
	private int sendLocal(MessageType message) {
		...
		((JSONMessageExtractor) worker).processMessage(new InetSocketAddress(
	...
	private int stampHeaderAndSendBytes(Object id, String msg)
			throws IOException {
		String header = JSONMessageExtractor.getHeader(msg);
	...
	static class Main {
		...
		public static void main(String[] args) {
			...
			for (int i = 0; i < nNodes + 1; i++) {
				workers[i] = new JSONMessageExtractor(
			...
			try {
				...
				for (int i = 0; i < smallNNodes; i++) {
					niots[i] = new MessageNIOTransport<Integer, JSONObject>(i,
							snc, workers[i]);
				 ...
				 * successive writes do not "accidentally" benefit from
				 * concurrency, i.e., to check that OP_WRITE flags will be set
				 * correctly.
				 */
				...
				for (int i = 0; i < nNodes * seqTestNum; i++) {
					...
					System.out.println("Message " + i + " with msgNum "
							+ msgNum);
					((MessageNIOTransport<Integer, JSONObject>) niots[k])
							.sendToIDInternal(
									j,
									JSONify(msgNum++, "Hello from " + k
											+ " to " + j));
				}
				...
				class TX extends TimerTask {
					...
					TX(MessageNIOTransport<Integer, JSONObject> niot, int id,
							int m) {
						sndr = niot;
						rcvr = id;
						msgNum = m;
